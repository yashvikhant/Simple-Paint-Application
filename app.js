...
//double click listener
    canvas.addEventListener('dblclick', function (e) {
        ...
        dataPool.forEach(function (value) {
            if (pos.x>value[0][0] && pos.x<value[2][0] && pos.y>value[0][1] && pos.y<value[2][1]) {
                var newList = [];
                ...
                clearCanvas();
                dataPool.forEach(function (value2) {
                    reDrawTriangles(value2[0][0], value2[0][1], value2[2][0], value2[2][1], value2[4]);
                });
                return true;
            }
        });
...
//movement listener
    canvas.addEventListener('mousemove', function (e) {
        ...
        if (isActive && flag) {
            ...
            reDrawTriangles(startCoordinates.x, startCoordinates.y, endCoordinates.x, endCoordinates.y, randomRGB);
            dataPool.forEach(function (value) {
                reDrawTriangles(value[0][0], value[0][1], value[2][0],value[2][1], value[4]);
            });
        } else if (isActive) {
            ...
            item[3][0] += difX;
            item[3][1] += difY;
            reDrawTriangles(item[0][0], item[0][1], item[2][0], item[2][1], item[4]);
            dataPool.forEach(function (value) {
...
//click-release listener
    canvas.addEventListener('mouseup', function (e) {
        ...
        if (!flag) {
            ...
            endCoordinates = mousePos;
            drawTriangle(startCoordinates.x, startCoordinates.y, endCoordinates.x, endCoordinates.y);
        }
...
// function to get current mouse position
    ...
    function reDrawTriangles(x1, y1, x2, y2, color) {
        context.beginPath();
    ...
    function drawTriangle(x1, y1, x2, y2) {
        ...
        dataPool.push([[x1, y1],[x1, y2],[x2, y2], [x2, y1], [context.fillStyle]]);
